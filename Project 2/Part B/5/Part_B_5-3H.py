...
def rnn_model(x,keep_chance):
  ...
  _, encoding = tf.nn.static_rnn(cell_with_dropout, char_list, dtype=tf.float32)
...
def main():
  ...
  for p in range(1,10):
    ...
    with tf.Session() as sess:
      ...
      try:
          prev_data_test_accs = np.loadtxt('/content/gdrive/My Drive/Colab Notebooks/b3h_test_accs.txt',ndmin=2)
          np_test_accs = np.append(prev_data_test_accs,np_test_accs,axis=0)

          prev_data_training_cost = np.loadtxt('/content/gdrive/My Drive/Colab Notebooks/b3h_training_cost.txt',ndmin=2)
          np_training_costs = np.append(prev_data_training_cost,np_training_costs,axis=0)

          prev_data_training_accs = np.loadtxt('/content/gdrive/My Drive/Colab Notebooks/b3h_training_acc.txt',ndmin=2)
          np_training_accs = np.append(prev_data_training_accs,np_training_accs,axis=0)

          prev_times = np.loadtxt('/content/gdrive/My Drive/Colab Notebooks/b3h_times.txt',ndmin=2)
          np_times = np.append(prev_times, np_times)

          np.savetxt('/content/gdrive/My Drive/Colab Notebooks/b3h_training_cost.txt',np_training_costs)
          np.savetxt('/content/gdrive/My Drive/Colab Notebooks/b3h_test_accs.txt',np_test_accs)
          np.savetxt('/content/gdrive/My Drive/Colab Notebooks/b3h_training_acc.txt',np_training_accs)
          np.savetxt('/content/gdrive/My Drive/Colab Notebooks/b3h_times.txt',np_times)
