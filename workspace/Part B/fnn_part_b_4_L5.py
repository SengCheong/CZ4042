...
def evaluate_fnn_param(params, first_layer_neurons):
    ...
    h1_dropout = tf.nn.dropout(hidden_1, keep_prob)
    ...
    # ~~~~~~~~~~~~~~~~~~ Hidden layer ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    # Hidden Layer, relu
    weights_h3 = tf.Variable(tf.truncated_normal([hidden_neurons, hidden_neurons],stddev=1.0 / np.sqrt(float(NUM_FEATURES))),name='weights')
    biases_h3 = tf.Variable(tf.zeros([hidden_neurons]),name='biases')
    hidden_3 = tf.nn.relu(tf.matmul(h2_dropout, weights_h3) + biases_h3)
    h3_dropout = tf.nn.dropout(hidden_3, keep_prob)
    # ~~~~~~~~~~~~~~~~~~ end of hidden layer ~~~~~~~~~~~~~~~~~~~~~~~
    ...
    y = tf.matmul(h3_dropout, weights_o) + biases_o
    # ~~~~~~~~~~~~~~~~~~ end of output layer ~~~~~~~~~~~~~~~~~~~~~

    #~~~~~~~~~~~~~~~~~~~ learning section ~~~~~~~~~~~~~~~~~~~~~~~
    regularization = tf.nn.l2_loss(weights_h1) + tf.nn.l2_loss(weights_o) + tf.nn.l2_loss(weights_h2)   + tf.nn.l2_loss(weights_h3)
    ...
    return (training_err,testing_err,True)
